...
#TODO add arguement for SampleSheet input
#TODO pull sample number from project spec to check it matches number of samples in SS

df = pd.read_excel(args.spec,sheet_name="Project Table",engine='openpyxl')
...
annotation = value_pull("Annotation Type")
depth_t = value_pull("Depth Target")
depth_spec = value_pull("Depth Spec")
h_removal = value_pull("Host Removal, Host")
ss_pull(df_ss)

print ("Are there pooled Samples? {}".format(pooled))
print ("Remove Host correct: PS {},SS {} {}".format(h_removal,r_host,host))
print ("Platform: PS {},SS {}".format(annotation,platform))
...
print ("Number of samples in Project Spec: {}".format(project_s_samp))
